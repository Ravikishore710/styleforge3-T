...
def compute_r1(discriminator, real_img, gamma: float, r1_interval: int) -> tf.Tensor:
    ...
    R1 is applied every r1_interval D-steps at r1_interval x strength so the
    time-averaged gradient matches continuous R1 with weight gamma.
    """
...
def adjust_betas_for_lazy_reg(beta1: float, beta2: float, eps: float,
    ...
    c = interval / (interval - 1); betas <- betas**c; eps <- eps / c.
    Returns (beta1, beta2, eps).
    """
    if interval <= 1:
